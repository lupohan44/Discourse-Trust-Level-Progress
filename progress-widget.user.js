...
(async () => {
  ...
  function buildUI() {
    ...
    $('#ld-btn').addEventListener('mouseenter',
      () => $('#ld-popup').classList.add('show'));
    $('#ld-container').addEventListener('mouseleave',
      () => $('#ld-popup').classList.remove('show'));
  ...
  async function refresh() {
    ...
    try {
      ...
      if (tl >= 3) { $('#ld-msg').innerText = 'TL3+ widget hidden'; return; }
